...
function validateNTPIP($nip)
{
    if ($nip != "") {
		...
		if (preg_match($regexp, $nip)) {
			$tempIP = explode(".",$nip);
			if ($tempIP[0] != 10) {
				return "First Octet needs to start with 10!<br />";
			} else {
				return "";
			}
		} else {
        ...
        return "Please enter in an the NTP Server IP which should be the IP of the MetroNID!<br />";
	}
}
// Validate Inside IP
